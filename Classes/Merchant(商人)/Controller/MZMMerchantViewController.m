...
@interface MZMMerchantViewController ()<UITableViewDataSource,UITableViewDelegate,JSDropDownMenuDataSource,JSDropDownMenuDelegate>
{
...
@implementation MZMMerchantViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    
//    self.tableView.frame = CGRectMake(0, 30, WIDTH, HEIGHT - 30);
    [self prepareNav];
...
- (void)prepareDataSource
{
    ...
    for (int i = 0 ; i < 10; i++) {
        MZMMerchantModel * model = [[MZMMerchantModel alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MZMMerchantCell * cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[MZMMerchantCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    
    //取出数据
    MZMMerchantModel * model = [_dataSource objectAtIndex:indexPath.row];
