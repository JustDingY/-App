...
#import "MZMFooterView.h"
#import "MZMCollectionViewCell.h"
#import "MZMDetailViewController.h"
#import "UIViewAdditions.h"
#import "MZMMainModel.h"
#import "MZMDetailModel.h"
#import "UIImageView+WebCache.h"

@interface MZMMoreViewController ()
...
@property (nonatomic, retain) MZMDetailViewController * secondVC;
@property (nonatomic, assign) BOOL isSelected;
@property (nonatomic, retain) NSIndexPath * index;
@property (nonatomic, retain) NSMutableIndexSet * indexSet;

//因为本地直接写无法接收文件对象而写的两个属性，但是这两个属性不用写成懒加载
...
@implementation MZMMoreViewController
...
- (void)viewDidLoad
{
    ...
    //注册cell
    [self.collectionView  registerClass:[MZMCollectionViewCell class] forCellWithReuseIdentifier:@"cell"];
    
    //注册页脚
    [self.collectionView registerClass:[MZMFooterView class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"footer"];
    
    //创建小视图控制器, 并作为本视图控制器的子视图控制器
    //把着两句代码写到这里就会只走一次, 不会说创建很多视图控制器
    self.secondVC = [MZMDetailViewController new];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MZMCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    ...
    if (self.listAry.count > 0) {
        ...
        MZMMainModel *main = self.listAry[indexPath.row + indexPath.section * kItem_Number];
...
//{
//    MZMFooterView *footerView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"footer" forIndexPath:indexPath];
//    
//    //将第二个视图控制器添加到区尾上
...
- (void)getDataFromServe
{
    ...
    if ([filer fileExistsAtPath:filePath] || filePathLocal) {
        ...
        for (NSDictionary *dic in dataAry) {
            ...
            MZMMainModel *main = [[MZMMainModel alloc] init];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (NSDictionary *dic in main.secondgrade) {
        ...
        MZMDetailModel *detail = [[MZMDetailModel alloc] init];
